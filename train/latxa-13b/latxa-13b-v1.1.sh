...
#SBATCH --output=.slurm/latxa-13b-v1.1.out
#SBATCH --error=.slurm/latxa-13b-v1.1.err
#SBATCH --exclusive
#SBATCH --requeue
...
SAVE_PATH=/leonardo_scratch/large/userexternal/jetxaniz/Llama-2-13b-neox-eus-v1.1
if [ -d "$SAVE_PATH" ]; then
    LOAD_PATH=$SAVE_PATH
    FINETUNE="false"
    CONF_NAME="continue"
...
printf '{\n "save": "%s",\n "load": "%s",\n "finetune": %s\n}' ${SAVE_PATH} ${LOAD_PATH} $FINETUNE \
    >${HOME}/llama-eus/leonardo/configs/save_load/${CONF_NAME}_13B_eus_v1.1.yml
...
python $TRAIN_PATH/deepy.py $TRAIN_PATH/train.py \
    ...
    data/latxa-v1.1.yml \
    save_load/${CONF_NAME}_13B_eus_v1.1.yml
