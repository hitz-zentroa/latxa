...
SAVE_PATH=$SCRATCH/Llama-2-70b-neox-eus-v1.1
if [ -d "$SAVE_PATH" ]; then
    LOAD_PATH=$SAVE_PATH
    FINETUNE="false"
    CONF_NAME="continue"
...
printf '{\n "save": "%s",\n "load": "%s",\n "finetune": %s\n}' ${SAVE_PATH} ${LOAD_PATH} $FINETUNE \
    >${HOME}/llama-eus/leonardo/configs/save_load/${CONF_NAME}_70B_eus_v1.1.yml
...
LOG_DIR=$SCRATCH/.wandb/llama-2-70b_v1.1_$(date "+%F")
mkdir -p $LOG_DIR
...
python $TRAIN_PATH/deepy.py $TRAIN_PATH/train.py \
    ...
    deepspeed/zero1.yml \
    hyperparameters/70B_v1.1.yml \
    data/latxa-v1.1.yml \
    save_load/${CONF_NAME}_70B_eus_v1.1.yml
